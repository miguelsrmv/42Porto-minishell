...
enum e_QuoteType { OUT_QUOTE, IN_QUOTE, IN_DQUOTE };

enum e_PipeType { OUT_PIPE, IN_PIPE };

enum e_TokenType { SEPARATOR, REDIRECT, STRING, REDIRECT_TARGET };
...
enum e_BuiltinType { EXPORT, CD };

enum e_ExpandType { PID, EXIT_STATUS, NUMBER, ATSIGN, HASHTAG };
...
void print_to_fd(int fd, char *error, char *extra_error, t_memptr memptr);
...
void set_cmd(t_token *lexer_sublist, t_command_table **command_table,
			 t_memptr memptr);
void create_command_table(t_token *lexer_list, t_command_table **command_table,
						  t_memptr memptr);
...
void fill_full_redir(t_token *current, t_command_table **command_table,
					 t_memptr memptr);
void initialize_command_table(t_command_table **command_table, int total_redir,
							  t_memptr memptr);
...
enum e_ValidType set_redirs(int **pipe_fd, t_command_table **command,
							t_memptr memptr);
void set_input_redir(int **pipe_fd, t_command_table **command, t_memptr memptr);
...
void relative_check_executables(t_command_table *current, char **path_list,
								t_memptr memptr);
void check_executables_subfunc(t_command_table *current, char *test_command);
...
void process_check(t_command_table **command_table, char **envp,
				   t_memptr *memptr);
...
bool command_check(t_command_table **command, char **path, t_memptr *memptr);
...
void fill_with_nonnullstrings(char **final_cmd, char **cmd_before_expansion,
